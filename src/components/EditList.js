...
const EditList = ({ setState, state }) => {
    ...
    const handleChange = (e, _index) => {
        ...
        if (index !== -1) {
            ...
            const SingleAmount = newItemList[index].itemQuantity * newItemList[index].itemRate;
    ...
    return (
        <>
            {items?.map((item) => {
                return (
                    <div key={item.index} className='relative multiple bg-gray-100 flex justify-between border-b-[1px] hover:bg-gray-300 border-b-gray-800 space-x-1 py-1 px-3 group' >
                        ...
                        {/* <TextField sx={{width:"20%"}} InputProps={{ sx: { "& input": { fontSize: "12px", color: 'black', textAlign: 'center', width: '150px', } } }} variant="standard" type="number" onChange={(e) => handleChange(e, item.index)} value={item.itemCgst} name="itemCgst" placeholder="%" />
                        <TextField sx={{width:"20%"}} InputProps={{ sx: { "& input": { fontSize: "12px", color: 'black', textAlign: 'center', width: '150px', } } }} variant="standard" type="number" onChange={(e) => handleChange(e, item.index)} value={item.itemSgst} name="itemSgst" placeholder="%" /> */}
            ...
            <EditBottomInvoice setState={setState} state={state} />
        </>
    );
};
